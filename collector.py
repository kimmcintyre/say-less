...
class Collector:
    def __init__(
        self, config_path: str | None, service_account_path: str | None
    ) -> None:
        SCOPES: list[str] = ["https://www.googleapis.com/auth/youtube"]
        creds: Credentials = Credentials.from_service_account_file(
            service_account_path or settings.DEFAULT_SERVICE_ACCOUNT_FILE, scopes=SCOPES
        )
    ...
    def _get_uploads_playlist_id(
        self, yt_service: Resource, channel_handle: str
    ) -> str:
    ...
    def _get_shorts_ids_from_shorts_playlist(
        self, yt_service: Resource, shorts_playlist_id: str, max_results: int
    ) -> list[str]:
    ...
    def _get_recent_videos_from_uploads(
        self, yt_service: Resource, upload_id: str, max_results: int | None
    ) -> list[Video]:
        ...
        shorts_playlist: list[str] = self._get_shorts_ids_from_shorts_playlist(
            yt_service, shorts_playlist_id, max_results
        )
        playlist_items = yt_service.playlistItems()
        result = playlist_items.list(
    ...
    def _get_yesterdays_videos(
        self, yesterdayDate: datetime, today: datetime, videos: list[Video]
    ) -> list[Video]:
        """Returns the list of details for videos published yesterday"""
        return [
            video
            for video in videos
            if (video.published_at > yesterdayDate and video.published_at < today)
        ]
    ...
    def get_youtube_videos(self, max_results: int) -> Table:
        ...
        self.log.info(
            f"Retrieving videos in the date range: {yesterdayDateStr} to {todayStr}"
        )
        ...
        for handle in self.channel_handles:
            ...
            videos_data: list[Video] = self._get_recent_videos_from_uploads(
                self.yt_service, playlist_id, max_results
            )
            videos_data: list[Video] = self._get_yesterdays_videos(
                yesterdayDate, today, videos_data
            )
