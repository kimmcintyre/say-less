...
if __name__ == "__main__":
    ...
    parser_gather = subparsers.add_parser(
        "gather",
        help="Gather YouTube links from the specified channel handles and saves them to a specified google sheet",
    )
    parser_gather.add_argument(
        "--configPath",
        type=str,
        help='Path to config file. Defaults to "./local/configs.json"',
    )
    parser_gather.add_argument(
        "--serviceAccountPath",
        type=str,
        help='Path to service account file. Defaults to "./local/secrets/service-account.json"',
    )
    parser_gather.add_argument(
        "--maxResults",
        type=int,
        help="Returns the lastest N videos to return per channel. Default value is the lastest 50 videos",
    )
